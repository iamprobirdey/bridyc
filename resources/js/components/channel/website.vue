  ...
  <div>
    ...
    <transition name="slide-fade">
      <div v-if="websiteOnPresent">
        ...
        <button class="btn btnwebdes rounded-0" @click="editMyWebsiteLink()">
          <i class="fa fa-pencil" aria-hidden="true"></i> Change website url
        </button>
      </div>
    </transition>
    <form @submit.prevent="getFormData()" v-if="!websiteOnPresent">
      <div
        class="form-group"
        :class="{
          'has-error': errors.has('websiteLinkError') || websiteLinkError != '',
        }"
      >
  ...
  methods: {
    getFormData() {
      this.$validator.validate().then((result) => {
        if (result) {
          if (this.websiteLinkFromServer != "") this.url = "/api/website/edit";
          axios
            ...
            .then((response) => {
              if (response.data.message === true) {
                ...
                this.websiteLinkFromServer = response.data.data.website_link;
                this.websiteOnPresent = true;
              }
            })
            .catch((errors) => {
              this.websiteLinkError = errors.response.data.message;
            });
        }
      });
    },
