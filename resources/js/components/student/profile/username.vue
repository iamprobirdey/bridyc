    ...
    <div v-if="userNameEditing" class="container mx-auto">
      <form @submit.prevent="userNameForm()">
        <div
          ...
          :class="{
            'has-error':
              errors.has('userNameError.username') ||
        ...
        >
          ...
          >
          <span
            v-show="userNameError.username != ''"
            class="help is-danger text-center"
            >{{ userNameError.username }}</span
          >
        </div>
        <div class="ml-5">
          <button type="btn" class="btn btn-warning" @click="cancelEditing()">
            Cancel
          </button>
