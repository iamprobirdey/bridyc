<header>
    <nav class="navbar navbar-expand-lg fixed-top px-3">
        <a class="navbar-brand" href="{{url('/')}}"><img class="imglogo img-fluid align-middle" src="/images/custom/bridyc-logo.svg" alt="Bridyc logo"></a>
        ...
        <div class="collapse navbar-collapse" id="collapsibleNavbar">
            ...
            <a class="btn btndashboard m-2" href="{{url('/edit/student/profile/'.auth()->user()->username)}}"><i class="fa fa-user-circle-o" aria-hidden="true"></i> Profile</a>
            @endif

            @if (auth()->check() && auth()->user()->isTeacher())
            <a class="btn btndashboard m-2" href="{{url('/edit/teacher/profile/'.auth()->user()->username)}}"><i class="fa fa-user-circle-o" aria-hidden="true"></i> Profile</a>
            @endif


            @if (auth()->check() && auth()->user()->isInstitute())
            @if (current_user()->channel != null)
            <a class="btn btndashboard m-2" data-trigger="focus" data-toggle="popover" data-placement="bottom" href="{{route('channel.index',current_user()->channel->title)}}">
                <i class="fa fa-th-large" aria-hidden="true"></i> My Dashboard
            </a>
            @else
            <a class="btn btndashboard m-2" data-trigger="focus" data-toggle="popover" data-placement="bottom" href="{{route('verification')}}">
